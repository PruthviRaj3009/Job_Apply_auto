...
def _classify_linkedin_text_answer(
    ...
    Returns None when no rule matches, so the caller can stop and ask the
    user rather than submit an invented answer.
    """
    q = label
    ...
    if "name" in q and "company" not in q:
        return cfg.name
    # No classifier matched. Returning a guess here (the old code answered
    # "Yes" to anything unrecognised, and invented an age) puts statements
    # the candidate never made onto a real application. Unknown means
    # unknown: the caller pauses and asks the user.
    return None
...
def _classify_linkedin_select(
    ...
    if yes and no and len(real) <= 3:
        ...
        if re.search(r"certificat|certified", q):
            certs = [c.lower() for c in af.get("certifications", [])]
            return yes if any(c and c in q for c in certs) else no
        # Everything else on a yes/no gate is a statement about the
        # candidate we have no basis for. Relocation, background-check
        # consent and work authorisation are commitments only the user
        # can make, so leave it unanswered and let the caller ask.
        return None
...
async def _apply_linkedin(page: Page, cfg: AppConfig, cover_note: str) -> dict[str, Any]:
    ...
    try:
        ...
        # Screening questions no rule could answer. A non-empty list blocks
        # submission: an application is only sent when every question on it
        # was answered from the user's own data.
        unanswered: list[str] = []
        ...
        for step in range(10):
            ...
            # --- Gate: never advance past a question we could not answer ---
            # Clicking Next/Submit with blanks left behind either submits an
            # incomplete application or lets LinkedIn's own defaults stand in
            # for the candidate. Stop here and hand the questions back so the
            # user can answer them and the job can be retried.
            if unanswered:
                return {
                    "success": False,
                    ...
                    "error": (
                        ...
                        f"{len(unanswered)} screening question(s) could not be "
...
async def _apply_naukri(page: Page, cfg: AppConfig, cover_note: str) -> dict[str, Any]:
    ...
    try:
        ...
        qa_log: list[dict[str, str]] = []
        # Chatbot questions no rule could answer — a non-empty list stops the
        # flow before the application is sent.
        unanswered: list[str] = []
        answered = 0
        ...
        for step in range(15):
            ...
            if has_radios:
                chosen = await page.evaluate("""(config) => {
                    ...
                    if (/notice/.test(q) && /negotia/.test(q)) {
                        ...
                        // Willingness to relocate is a commitment only the
                        // user can make — leave it for them if no configured
                    ...
                    // No rule matched. Clicking the first option would put a
                    // statement the candidate never made on the application,
                    // so report it back unanswered instead.
                ...
                if chosen and chosen.get("ok"):
                    ...
                    unanswered.append(question[:160])
            ...
            if has_contenteditable:
                ...
                # Willingness / commitment questions (relocation, onsite,
                # joining date) are promises only the user can make, and an
                # unrecognised question is by definition one we have no
                # answer for. Both leave `value` empty so the flow below
                # records the question and stops short of submitting.
                else:
                    value = ""
            ...
            # === 4b) GATE: stop rather than send an unanswered question ===
            # Naukri's chatbot advances one question at a time, so clicking
            # Save on a question we could not answer commits a blank or a
            # site default in the candidate's name. Hand it back instead.
            if unanswered:
                return {
                    "success": False,
                    "needs_user_input": True,
...
async def apply_job(
    ...
    # Track in DB. A run that paused for the user is neither applied nor
    # failed — recording it as "failed" would hide the fact that the job is
    # still actionable once the questions are answered.
