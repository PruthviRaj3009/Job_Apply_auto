...
# pywinauto drives the native Windows file-upload dialog. It has no Linux
# build, so importing it unconditionally made the whole module — and every
# module that imports it — unimportable in Docker. Degrade instead: the
# dialog-driving paths raise a clear error if reached off Windows.
try:
    ...
    Application = findwindows = send_keys = None
    PYWINAUTO_AVAILABLE = False

    class ElementNotFoundError(Exception):
        """Stand-in so except-clauses below stay valid without pywinauto."""
...
def upload_file_via_dialog(target_dialog_title: str, file_path: str) -> bool:
    if not PYWINAUTO_AVAILABLE:
        raise RuntimeError(
            "Native file-dialog upload requires pywinauto (Windows only). "
            "On other platforms use the Playwright engine, which sets the "
            "file input directly and needs no OS dialog."
        )
    """
