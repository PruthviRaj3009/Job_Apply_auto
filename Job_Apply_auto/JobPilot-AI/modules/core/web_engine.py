...
from modules.core.form_filler import UserInputRequired
...
def run_job(JOB_URL: str):
    ...
    for iteration in range(MAX_ITERATIONS):
        ...
        try:
            resolved = interactor.resolve_parsed_data()
        except UserInputRequired as exc:
            # A question on this form is not answerable from the user's own
            # data. Stop with the browser left open so they can see where the
            # run halted, and report the question rather than submitting.
            logger.warning("⏸️  Paused — user input required: %s", exc.question)
            driver.quit()
            return {"status": "waiting_for_user", "question": exc.question, "url": JOB_URL}
        if not resolved:    # Resolution response
