...
def init_db():
    ...
    c.execute('''
        CREATE TABLE IF NOT EXISTS job_queue (
            ...
            status TEXT CHECK(status IN ('new', 'active', 'success', 'failed', 'waiting_for_user')) NOT NULL,
            note TEXT,
            created_at TEXT DEFAULT CURRENT_TIMESTAMP,
            updated_at TEXT DEFAULT CURRENT_TIMESTAMP
        )
    ''')
    # Additive migration: existing job_store.db files predate these columns.
    cols = {r[1] for r in c.execute("PRAGMA table_info(job_queue)")}
    if "note" not in cols:
        c.execute("ALTER TABLE job_queue ADD COLUMN note TEXT")
    conn.commit()
    conn.close()
...
class JobUpdate(BaseModel):
    url: str
    status: str
    # Why a job is waiting — the unanswered question, for the user to resolve.
    note: str | None = None
...
def update_job(update: JobUpdate = Body(...)):
    ...
    c = conn.cursor()
    c.execute(
        "UPDATE job_queue SET status=?, note=?, updated_at=CURRENT_TIMESTAMP WHERE url=?",
        (status, update.note, url),
    )
    if c.rowcount == 0:
        c.execute("INSERT INTO job_queue (url, status, note) VALUES (?, ?, ?)", (status, update.note, url))
    conn.commit()
    conn.close()
